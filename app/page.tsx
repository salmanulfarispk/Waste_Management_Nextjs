...
import { getAllRewards, getRecentReports, getWasteCollectionTask } from '@/lib/actions'
  ...
  return (
    <div className={`container mx-auto px-4 py-16 ${poppins.className}`}>
      <section className="text-center mb-20">
        ...
        {!loggedIn ? (
          <button onClick={login} className="btn bg-green-600  hover:bg-green-700 text-white text-lg py-3 px-8 rounded-full font-medium transition-all duration-300 ease-in-out transform hover:scale-105">
            Get Started
            <ArrowRight className="ml-2 h-5 w-5" />
          </button>
        ) : (
          <Link href="/report">
            <button className="btn bg-green-600 hover:bg-green-700 text-white text-lg py-2 px-3 rounded-3xl">
              Report Waste
              <ArrowRight className="ml-2 h-5 w-5" />
            </button>
          </Link>
